...
const htmlFiles = ["index.html", "players.html", "fixtures.html", "match.html", "history.html", "appearances.html", "goals.html", "assists.html", "news.html", "news-article.html"];
...
expect(Object.keys(browserData.teamBadges).length === matchTeams.length, "site-data.js 的队徽映射数量不正确");
...
for (const item of newsDocument.items) {
  ...
  expect(Boolean(item.localImage), `${item.id} 缺少本地资讯图片`);
...
console.log(`官方资讯：${newsDocument.items.length} 条，当前中文翻译 ${newsDocument.items.filter((item) => item.titleZh && item.descriptionZh).length} 条，本地图片完整`);
